...
Object.assign(FinalBoss.prototype, {
  ...
  applyMovement() {
    if (
      !this.character ||
      !Number.isFinite(this.character.x) ||
      !Number.isFinite(this.character.y)
    ) {
  ...
  applyStyleMovement(vector) {
    if (this.swimStyle === "aggressive") this.applyAggressiveMovement(vector);
      ...
      this.applyDefensiveMovement(vector);
  ...
  applyVerticalTracking(vector) {
    ...
    const verticalStep = Math.min(
      this.floatingSpeed * 1.2 * upwardBoost,
      verticalDistance * 0.26,
    );
  ...
  applyCircleMovement(vector) {
    ...
    this.floatingTargetX =
      this.character.x + Math.cos(angle + 0.05) * desiredDistance;
    this.floatingTargetY =
      this.character.y + Math.sin(angle + 0.05) * desiredDistance;
  ...
  moveToFloatingTarget() {
    ...
    const targetDist = Math.sqrt(
      targetDistX * targetDistX + targetDistY * targetDistY,
    );
