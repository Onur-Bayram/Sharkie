...
class Pufferfish extends MovableObject {
  ...
  buildPufferImagePaths() {
    ...
    this.IMAGES_SWIM = Array.from(
      { length: 5 },
      (_, i) => `${base}/1.Swim/${v}.swim${i + 1}.png`,
    );
    this.IMAGES_TRANSITION = Array.from(
      { length: 5 },
      (_, i) => `${base}/2.transition/${v}.transition${i + 1}.png`,
    );
    this.IMAGES_BUBBLESWIM = Array.from(
      { length: 5 },
      (_, i) => `${base}/3.Bubbleeswim/${v}.bubbleswim${i + 1}.png`,
    );
  ...
  tickPufferMovement() {
    ...
    if (Math.abs(this.y - this.targetY) > 1) {
      this.y +=
        this.y < this.targetY ? this.verticalSpeed : -this.verticalSpeed;
    }
  }
  ...
  isTransitionState() {
    return (
      this.state === "transitionToBubble" || this.state === "transitionToSwim"
    );
  }
