Object.assign(World.prototype, {
  ...
  updateCameraX() {
    this.cameraX =
      this.character.x - this.GAME_WIDTH / 2 + this.character.width / 2;
  ...
  drawFinalBoss() {
    ...
    if (!shouldFlip) {
      this.ctx.drawImage(
        ...
        this.finalBoss.x,
        this.finalBoss.y,
        this.finalBoss.width,
        this.finalBoss.height,
      );
      return;
    }
    this.ctx.save();
    this.ctx.translate(
      this.finalBoss.x + this.finalBoss.width,
      this.finalBoss.y,
    );
    this.ctx.scale(-1, 1);
    this.ctx.drawImage(
  ...
  isCollidingBoss(character, boss) {
    ...
    const overlapX =
      Math.min(cBox.right, bBox.right) - Math.max(cBox.left, bBox.left);
    const overlapY =
      Math.min(cBox.bottom, bBox.bottom) - Math.max(cBox.top, bBox.top);
