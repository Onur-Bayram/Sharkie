Object.assign(World.prototype, {
  ...
  checkBossIntroSound(hadStartedIntro) {
    if (
      !hadStartedIntro &&
      this.finalBoss.hasStartedIntro &&
      !this.bossIntroSoundPlayed
    ) {
  ...
  spawnBossFightPoisonBottle() {
    ...
    const spawnX = Math.max(
      minX,
      Math.min(maxX, this.character.x + (Math.random() * 420 - 210)),
    );
    this.animatedPoisonBottles.push(
      this.createBossBottle(spawnX, -220 - Math.random() * 140),
    );
   ...
   */
  canSpawnBossBottle() {
    if (!this.finalBoss || this.finalBoss.isDead || !this.bossLevelLocked)
      return false;
    if (Date.now() - this.lastBossBottleDropAt < this.bossBottleDropCooldown)
      return false;
    const active = this.animatedPoisonBottles.filter(
      (b) => b.fromBossFight && !b.collected,
    ).length;
  ...
  checkBossBodyCollision() {
    if (!this.finalBoss || this.finalBoss.isDead || this.character.isDead)
      return;
    ...
    if (!this.canCharacterTakeContactDamage()) return;
    const damage = biteHit
      ? 38
      : this.finalBoss.state === "attacking"
        ? 35
        : 22;
  ...
  isBossBiteHit(character, boss) {
    ...
    const overlapX =
      Math.min(cBox.right, mBox.right) - Math.max(cBox.left, mBox.left);
    const overlapY =
      Math.min(cBox.bottom, mBox.bottom) - Math.max(cBox.top, mBox.top);
  ...
  canCharacterTakeContactDamage() {
    ...
    return (
      !this.character.isHurt || currentTime - this.character.lastHitTime > 600
    );
  },
  ...
  handleBossEndState() {
    if (
      ...
      !this.finalBoss.deadAnimationFinished
    )
      return;
  ...
  collectStaticPoisonBottles() {
    for (let i = this.poisonBottles.length - 1; i >= 0; i--) {
      ...
      this.character.poison = Math.min(
        this.character.poison + this.SMALL_POISON_BOTTLE_VALUE,
  ...
  collectAnimatedPoisonBottles() {
    for (let i = this.animatedPoisonBottles.length - 1; i >= 0; i--) {
      ...
      this.character.poison = Math.min(
        this.character.poison + this.LARGE_POISON_BOTTLE_VALUE,
        this.character.maxPoison,
      );
      this.poisonBar.setPercentage(this.character.poison);
      this.audioManager.playSfx("potion");
